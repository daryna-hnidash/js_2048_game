...
function isLost() {
    ...
    for(let r = 0; r < rows - 1; r++)for(let c = 0; c < columns; c++){
        if (gameField[r][c] === gameField[r + 1][c]) return false;
    }
    return true;
}
...
function slide(row) {
    ...
    for(let c = 0; c < columns - 1; c++)if (slidedRow[c] === slidedRow[c + 1]) {
        slidedRow[c] *= 2;
        slidedRow[c + 1] = 0;
        score += +slidedRow[c] | 0;
        gameScore.innerText = score;
...
function slideUp() {
    ...
    for(let c = 0; c < rows; c++){
        ...
        row = slide(row);
        for(let index = 0; index < rows; index++)fieldCopy[index][c] = row[index];
    }
    updateHandler(fieldCopy);
}
function slideDown() {
